 ...
 *   Lv 1   0 XP   实习老师小进 — 默认 Maia (v0.36.32 爸爸: 更适合讲题; Serena 挪 Lv8)
...
export const MASCOT_LEVELS: MascotLevel[] = [
  { level: 1, threshold: 0, title: "实习老师小进", unlocks: { voices: ["Maia"] } },
...
/** 当前佩戴的音色（v0.36.22 默认 Maia — 跟 TTS 统一 (爸爸觉得更适合讲题)；切到没解锁的会被忽略） */
export async function getEquippedVoice(studentId: string): Promise<string> {
  ...
  return state.unlockedVoices.includes(v) ? v : "Maia";
}
