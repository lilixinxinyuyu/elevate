...
export const PROMPTS = {
  ...
  "skillScope": {
    ...
    "_g4a_block_below": "==================== 数学 G4A 上册 19 个 skill (v0.36.20 补全, 综合复习/期末会用) ====================",
    "large_place_value": {
      "name": "数位顺序表、万/亿级认识",
      "term": "上册",
      "unitId": "G4A_U1_LARGE_NUMBERS",
      "definition": "认识亿以内、亿以上的数。数位顺序表（个十百千万十万百万千万亿…），每相邻两个计数单位间是十进关系。计数单位、数位、位数的区别。",
      "inScope": [
        "数位顺序表：个级（个十百千）/ 万级（万十万百万千万）/ 亿级",
        "某数位上的数字表示几个该计数单位",
        "10 个一万 = 一十万，10 个十万 = 一百万（满十进一）",
        "数的组成（如 305060 由 3 个十万 5 个千 6 个十组成）"
      ],
      "outOfScope": [
        "❌ 不考小数数位（下册）",
        "❌ 不考超过亿级的天文数字运算"
      ...
      "exampleStems": [
        "在 6 0 5 0 0 0 中，6 在什么位上，表示几个什么？",
        "8 个百万和 5 个万组成的数是多少？"
      ]
    },
    "large_read_write": {
      ...
      "term": "上册",
      "unitId": "G4A_U1_LARGE_NUMBERS",
      "definition": "亿以内 / 亿以上数的读写。从高位读起，分级（每 4 位一级）读，每级末尾 0 不读，中间连续几个 0 只读一个零。",
      "inScope": [
        "分级读数（万级、亿级）",
        "每级末尾的 0 不读，其他数位连续 0 只读一个'零'",
        "按级写数（先写万级 / 亿级再写个级，空位补 0）"
      ],
      "outOfScope": [
        "❌ 不考小数读写",
        "❌ 不考罗马数字 / 古代记数"
    ...
    "large_compare": {
      ...
      "term": "上册",
      "unitId": "G4A_U1_LARGE_NUMBERS",
      "definition": "比较亿以内 / 亿以上数的大小。位数多的数大；位数相同时从最高位起逐位比。",
      "inScope": [
        "位数不同：位数多的大",
        "位数相同：从最高位逐位比较",
        "用 > < = 连接"
    ...
    "large_rewrite_wan_yi": {
      ...
      "term": "上册",
      "unitId": "G4A_U1_LARGE_NUMBERS",
      "definition": "把整万 / 整亿的数改写成以'万'或'亿'作单位的数（精确改写，不是近似）。去掉末尾 4 个 0 加'万'，去掉 8 个 0 加'亿'。",
      "inScope": [
        "整万数改写成'万'作单位",
        "整亿数改写成'亿'作单位",
        "改写是精确的（≠ 四舍五入近似）"
      ...
      "outOfScope": [
        "❌ 改写跟'求近似数'不同（那是 large_approx_rounding）",
        "❌ 不考非整万数的改写"
      ],
      "keyFormulas": [
        "改写成万：去掉末尾 4 个 0，添'万'字",
        "改写成亿：去掉末尾 8 个 0，添'亿'字"
      ],
      "typicalContexts": [
        "120000 = ( )万",
        "改写成以亿作单位"
    ...
    "large_approx_rounding": {
      ...
      "term": "上册",
      "unitId": "G4A_U1_LARGE_NUMBERS",
      "definition": "用四舍五入法求一个数省略某一位后面的尾数得到近似数。看要省略位的后一位：满 5 进 1，不满 5 舍去，结果加'万'或'亿'。",
      "inScope": [
        "省略万位后面的尾数（四舍五入到万）",
        "省略亿位后面的尾数",
        "用'≈'号"
      ...
      "outOfScope": [
        "❌ 跟'精确改写'(large_rewrite_wan_yi) 区分",
        "❌ 不考小数的四舍五入（下册）"
      ],
      "keyFormulas": [
        "看省略位的下一位：≥5 进 1，<5 舍",
    ...
    "angle_types": {
      "name": "锐角、直角、钝角、平角、周角",
      "term": "上册",
      "unitId": "G4A_U2_LINES_ANGLES",
      "definition": "认识角的分类。按度数：锐角 <90°，直角 =90°，钝角 90°~180°，平角 =180°，周角 =360°。",
      "inScope": [
        "5 种角的度数范围判断",
    ...
    "angle_measure": {
      ...
      "term": "上册",
      "unitId": "G4A_U2_LINES_ANGLES",
      "definition": "用量角器量角的度数、画指定度数的角。中心对顶点、0 刻度线对一边，看另一边对的刻度（内外圈别读错）。",
      "inScope": [
        "量角的度数（内圈 / 外圈正确读数）",
        "画指定度数的角",
        "角的大小与边的长短无关，只跟张口大小有关"
      ],
      "outOfScope": [
        "❌ 不考量角器构造原理",
        "❌ 不考超过 180° 的角的测量"
      ...
      "exampleStems": [
        "量角器 0 刻度线对齐一边，另一边指向 60，这个角是多少度？",
        "角的两条边越长，角就越大。对吗？"
      ]
    },
    "int_mul_3_by_2": {
      "name": "三位数乘两位数笔算",
      "term": "上册",
      "unitId": "G4A_U3_MULTIPLICATION",
      "definition": "三位数乘两位数的竖式笔算。先用两位数个位去乘，再用十位去乘（积末位对齐十位），最后相加。",
      "inScope": [
        "三位数 × 两位数竖式",
        "因数中间 / 末尾有 0 的乘法",
        "积的位数估计"
    ...
    "int_mul_estimation": {
      ...
      "term": "上册",
      "unitId": "G4A_U3_MULTIPLICATION",
      "definition": "把因数看成接近的整十 / 整百数估算积的范围。判断够不够 / 大约多少。",
      "inScope": [
        "把因数四舍五入成整十整百再乘",
        "判断'钱够不够''座位够不够'类问题",
        "估算结果比实际大还是小的判断"
    ...
    "mixed_ops_brackets": {
      ...
      "term": "上册",
      "unitId": "G4A_U4_LAWS",
      "definition": "含小括号、中括号的四则混合运算。先算括号里（先小括号再中括号），再算乘除，最后算加减。",
      "inScope": [
        "运算顺序：括号 → 乘除 → 加减",
        "小括号内先算，再算中括号",
        "三步以内的混合运算"
    ...
    "distributive_law": {
      ...
      "term": "上册",
      "unitId": "G4A_U4_LAWS",
      "definition": "乘法分配律 (a+b)×c = a×c + b×c。两个数的和与一个数相乘，等于分别相乘再相加。逆用可简便计算。",
      "inScope": [
        "(a+b)×c = a×c+b×c 正用",
        "a×c+b×c = (a+b)×c 逆用简算",
        "(a-b)×c = a×c-b×c"
      ...
      "outOfScope": [
        "❌ 不跟结合律 / 交换律混（那是其他 skill）",
        "❌ 不考小数分配律（下册）"
      ],
      "keyFormulas": [
        "(a+b)×c = a×c + b×c",
        "a×c + b×c = (a+b)×c（逆用提取公因数）"
      ],
      "typicalContexts": [
        "用分配律简便计算",
        "填空补全分配律",
    ...
    "simplify_integer": {
      ...
      "term": "上册",
      "unitId": "G4A_U4_LAWS",
      "definition": "综合运用加法交换 / 结合律、乘法交换 / 结合 / 分配律进行简便计算。凑整是核心思路。",
      "inScope": [
        "加法交换 + 结合律凑整",
        "乘法交换 + 结合律凑整（如 25×4, 125×8）",
        "分配律简算",
        "减法性质 a-b-c=a-(b+c)"
      ],
      "outOfScope": [
    ...
    "grid_coordinates": {
      "name": "用数对（列，行）确定位置",
      "term": "上册",
      "unitId": "G4A_U5_POSITION",
      "definition": "用数对确定方格 / 座位中的位置。数对 (列, 行)：先看第几列（从左往右），再看第几行（从下往上）。",
      "inScope": [
        "读出某点的数对",
        "根据数对找位置",
        "(列, 行) 顺序：列在前行在后"
    ...
    "div_3_by_2_trial": {
      "name": "三位数除以两位数试商",
      "term": "上册",
      "unitId": "G4A_U6_DIVISION",
      "definition": "三位数除以两位数的笔算除法，用'四舍五入'把除数看成整十数试商。商写在哪一位、余数要比除数小。",
      "inScope": [
        "把除数看成整十数试商",
        "商的定位（够除从高位起）",
        "余数 < 除数的检验"
    ...
    "div_adjust_quotient": {
      ...
      "term": "上册",
      "unitId": "G4A_U6_DIVISION",
      "definition": "试商后发现商过大（余数 ≥ 除数或不够减）或商过小（余数 ≥ 除数）时调整商。四舍试商常偏大要调小，五入试商常偏小要调大。",
      "inScope": [
        "余数 ≥ 除数 → 商过小，调大",
        "乘积 > 被除数那部分 → 商过大，调小",
        "判断需要调商的情形"
    ...
    "speed_time_distance": {
      ...
      "term": "上册",
      "unitId": "G4A_U6_DIVISION",
      "definition": "速度、时间、路程三量关系。速度 = 路程 ÷ 时间，路程 = 速度 × 时间，时间 = 路程 ÷ 速度。速度单位的复合写法（千米/时）。",
      "inScope": [
        "三量互求",
        "速度单位复合写法（千米/时、米/分）",
        "整数范围的行程问题"
      ...
      "outOfScope": [
        "❌ 不考小数速度（下册 decimal_speed_distance）",
        "❌ 不考相遇 / 追及（中学或下册）"
      ],
      "keyFormulas": [
        "速度 = 路程 ÷ 时间",
      ...
      "exampleStems": [
        "汽车 3 小时行 240 千米，速度是多少？",
        "速度 60 千米/时，行 5 小时走多远？"
      ]
    },
    "negative_temperature": {
      "name": "温度中的负数",
      "term": "上册",
      "unitId": "G4A_U7_NEGATIVE",
      "definition": "在温度情境中认识负数。0℃ 是分界，零上为正、零下为负。比较温度高低、读写温度计。",
      "inScope": [
        "零上 / 零下温度的正负表示",
        "温度高低比较（负数越大越冷）",
        "温度计读数"
    ...
    "zero_not_pos_neg": {
      ...
      "term": "上册",
      "unitId": "G4A_U7_NEGATIVE",
      "definition": "正负数的意义与 0 的特殊地位。0 是正负数的分界，0 既不是正数也不是负数。用正负数表示相反意义的量。",
      "inScope": [
        "0 既不是正数也不是负数（判断题常考）",
        "用正负表示相反意义的量（收入 / 支出、上升 / 下降）",
        "正数都比 0 大，负数都比 0 小"
      ],
      "outOfScope": [
        "❌ 不考负数四则运算",
      ...
      "exampleStems": [
        "0 既不是正数，也不是( )。对吗？",
        "收入 200 元记作 +200，支出 50 元记作( )"
      ]
    },
    "probability_compare": {
      "name": "判断可能性大小",
      "term": "上册",
      "unitId": "G4A_U8_PROBABILITY",
      "definition": "用'一定''可能''不可能'描述事件，比较可能性大小。数量多的可能性大，数量少的可能性小。",
      "inScope": [
        "'一定 / 可能 / 不可能'判断",
        "比较摸到不同颜色球的可能性大小",
        "可能性大小跟数量多少的关系"
      ...
      "exampleStems": [
        "盒子里全是红球，摸出红球是( )发生（一定 / 可能 / 不可能）",
        "5 个红球 2 个白球，摸到哪种球可能性大？"
      ]
    },
    ...
    "_chinese_generic_below": "===== 题型级通用 scope（v0.36.14）：错别字/病句/仿写/阅读 跨单元共用，getSkillScope 按后缀 fallback（C4B_U1_TYPOS → C4B_TYPOS_GENERIC）=====",
    "C4B_TYPOS_GENERIC": {
      "name": "错别字辨识与改正",
      "term": "下册",
      "definition": "给一句话或一小段文字，里面有若干个错别字（同音字 / 形近字 / 多写少写笔画），找出并改正。期中常考题型，例「李强来信里有 3 处错别字，请找出来改正」。",
      "inScope": [
        "同音字混用（在 / 再、做 / 作、的 / 得 / 地）",
        "形近字混用（漂 / 飘、辨 / 辩 / 辫、即 / 既）",
        "4 年级下册课文常用词里的易错字",
      ...
      "keyFormulas": [
        "同音字看意思：「在」表方位/时间，「再」表又一次",
        "「的」+名词 / 「地」+动词 / 「得」+补语",
        "形近字看偏旁：「漂」三点水(漂流)、「飘」风字旁(飘扬)"
      ],
      "typicalContexts": [
        "一段书信 / 日记 / 短文里找错别字",
        "判断哪句话有错别字",
      ...
      "exampleStems": [
        "下面这段话有 3 个错别字，找出来并改正：「我做在教室里，在一次听老师讲课。」",
        "下列词语没有错别字的一组是：A. 飘流 B. 漂扬 C. 飘扬 D. 漂浮",
        "选出正确的字填空：他（的 / 地 / 得）跑（的 / 地 / 得）很快。"
      ]
    },
    "C4B_BADSENT_GENERIC": {
      "name": "病句辨识与修改",
      "term": "下册",
      "definition": "给一个有语病的句子，判断病因并改正。4 年级下册重点病句类型：语序颠倒、成分残缺、搭配不当、重复啰嗦、前后矛盾、用词不当。",
      "inScope": [
        "语序颠倒（「我把作业做完了」vs「我做完了把作业」）",
        "成分残缺（缺主语 / 缺谓语 / 缺宾语）",
        "搭配不当（「提高水平」对，「改进水平」错）",
        "重复啰嗦（「大约 30 个左右」「他大概可能会来」）",
        "前后矛盾（「全班同学都来了，只有小明没来」）",
        "用词不当（褒贬误用 / 关联词错）"
      ],
      "outOfScope": [
      ...
      "keyFormulas": [
        "改病句 4 步：读句子 → 找病因 → 改 → 再读一遍通不通",
        "「大约/左右/可能」表估计的词不能重复用",
        "搭配看动词宾语：提高(水平/成绩)、改进(方法)、改善(生活)"
      ],
      "typicalContexts": [
        "给一个病句，指出病因 + 改正",
        "4 选 1 选出没有语病的句子",
      ...
      "exampleStems": [
        "修改病句：「我大约用了 30 分钟左右完成了作业。」",
        "下列句子没有语病的一项是：A. 通过这次活动，使我懂得了道理。B. 我们要养成爱读书。C. 春天的公园是个美丽的地方。D. 他基本上全部做完了。",
        "「操场上有打篮球、踢足球和各种活动。」这句话错在哪？怎么改？"
      ]
    },
    "C4B_IMITATE_GENERIC": {
      "name": "仿写句子",
      "term": "下册",
      "definition": "给一个例句，仿照它的修辞手法或句式结构写一个新句子。4 年级下册重点：比喻句、拟人句、排比句、用指定词语 / 调动颜色感官的描写句。",
      "inScope": [
        "仿写比喻句（A 像 B）",
        "仿写拟人句（物 + 人的动作情感）",
        "仿写排比句（≥3 个结构相似分句）",
        "用颜色 / 感官词描写景物的句子",
        "用指定关联词造句（不但…而且 / 虽然…但是）"
      ],
      "outOfScope": [
        "❌ 不考整段仿写（只仿 1-2 句）",
        "❌ 不考夸张 / 对偶等高年级修辞",
      ...
      "keyFormulas": [
        "仿写 = 学结构 + 换内容（保留例句的句式骨架）",
        "比喻：本体 + 像/仿佛 + 喻体（要有相似点）",
        "拟人：给物加人的动作（笑、唱、招手、跳舞）",
        "排比：3 个以上「……的……」结构句"
      ],
      "typicalContexts": [
        "仿照例句写一句（同修辞）",
      ...
      "exampleStems": [
        "仿写句子：例「弯弯的月亮像一只小船」，用比喻写一句关于云的句子。",
        "仿照例句，用拟人手法写一句：例「春风轻轻地抚摸着大地」。",
        "用排比句仿写：「校园里有红的花，绿的草，______。」"
      ]
    },
    "C4B_READING_GENERIC": {
      "name": "阅读理解（短文 + 多问题）",
      "term": "下册",
      "definition": "给一篇 4 年级水平的短文（150-300 字），后面跟 2-4 个问题，考查信息提取、概括大意、词句理解、简单推断。multi_step 题型（一篇短文配多个小问）。",
      "inScope": [
        "信息提取（短文里直接找答案）",
        "概括段意 / 主要内容",
        "联系上下文理解词语 / 句子",
      ...
      "exampleStems": [
        "读短文回答：①短文主要写了什么？②第 2 段中「机灵」一词说明小松鼠怎样？③你从这件事懂得了什么道理？",
        "阅读《翠鸟》片段，回答：翠鸟的羽毛有哪几种颜色？",
        "读短文，根据内容填空 + 选择正确答案。"
      ]
    }
  },
  ...
  "gameTypeBySkill": {
    ...
    "_chinese_below": "===== 语文 G4B (v0.36.38): READING→word_problem_lab(短文多问), IMITATE→fill_blank(仿写), ORDER→sort_ladder+sentence_shuffle, POEM_RECITE→poem_cloze(古诗补字 C1), GLYPH→glyph_detective(字形侦探 C2). 显式 gameType 可强制指定任意 cluster 题型(poem_cloze/sentence_shuffle/glyph_detective). 其他(TYPOS/VOCAB/PINYIN/DICTATION/RHETORIC)走默认 plain_choice =====",
    "C4B_U1_READING": [
      {
        "type": "word_problem_lab",
        "weight": 3
