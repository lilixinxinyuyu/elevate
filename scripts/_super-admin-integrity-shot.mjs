/** Screenshot super-admin with data-integrity panel expanded. */
...
await page.evaluate(() => {
  ...
  const di = details.find((d) => (d.textContent ?? "").toLowerCase().includes("data integrity"));
  if (di) di.open = true;
});
await new Promise((r) => setTimeout(r, 2500));
await page.screenshot({ path: `${OUT}/10-data-integrity.png`, fullPage: false });
await browser.close();
console.log(`saved ${OUT}/10-data-integrity.png`);
